...
**(Optional)** `-c` or `--config-file-path` | A custom [configuration file](docs/customizing-evaluation-outputs.md) with custom settings. If the configuration file path argument is not provided, by default, the analyzer will check to see if a file is present in  "ExecutablePath/Configurations/Configuration.json".
